/*  Last Modified Time-stamp: <2014-08-26 13:59:36 mike_georgiou> */
...

{
  ...
  //First, define the integer indices that will be used by the
  //algorithm 
  int i,j,xx,k;
  ...
  //This vector corresponds to the  value  A*X, i.e. the
  //multiplication of A with the initial guess.
  double *T;
  T=dvector(1,N);
  ...
  /* Error analysis: The BCGC algorithm imlpemented herein uses the
     l^1 norm as a termination criteria norm.

     In particular, if X^i denotes the solution computed at the step
     i, then the termination criteria is
  ...
  */
  ...
  // A_norm will be used to compute the l^1 norm of matrix A
  double A_norm=0;

  for(i=1; i <= ELEMENTS; i++)
    {
      A_norm = A_norm  + (sL[i]*sL[i]);
    }
  A_norm=sqrt(A_norm);
  //Next, compute the l^1 norm of the vector b:

  //b_norm will be used to compute the l^1 norm of the vector b.
  double b_norm=0.;
  for(i=1; i<= N; i++)
    {
      b_norm = b_norm  + (D[i]*D[i]);
    }
  b_norm=sqrt(b_norm);

  // Next, proceed to the main Loop.
  /*******************************************************************
   ************Beginning the Numerical  Algorithm ********************
   ******************************************************************/
      ...
      if (j==0)
        {
          ...
          // Next, compute the norm of the error. For this, we first
          // compute the l^2 norm of the guess value X
      ...
      if (j==1)
        {
          ...
          for(i=1; i<=N; i++)
            {
              s1_norm = s1_norm + (s1[i]*s1[i]);

            }
          s1_norm=sqrt(s1_norm);
          ns1[j]=s1_norm;
