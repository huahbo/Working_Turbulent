 ...
 *  Last Modified: Time-stamp: <2014-09-04 17:36:05 mike_georgiou>
 *
 *
...
void Intermediate_Velocity_Corrector_2ND_Z(double***velocity_z_tilda,
                                           double*** residual_z,
  ...
  for (int k=0; k<ldz; k++){
    for (int i=0; i<ldx; i++){
      ...
      for (int j=0; j<ldy; j++){
        ...
        double pressure_gradient =(pressure[k+1][j][i]-pressure[k-1][j][i])/(2.*dz);
        ...
        if (j==0)
          {
            ...
            rhs[j+1] =
              dt/rho_new[k][j][i]
              *(rho[k][j][i]*velocity_z_old[k][j][i]/dt
